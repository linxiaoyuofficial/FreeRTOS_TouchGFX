...
//    __HAL_RCC_GPIOI_CLK_ENABLE();
//    /**LTDC GPIO Configuration
//    PI15     ------> LTDC_R0                  PJ7     ------> LTDC_G0
//    PJ0     ------> LTDC_R1                   PJ8     ------> LTDC_G1
//    PJ1     ------> LTDC_R2                   PJ9     ------> LTDC_G2
//    PJ2     ------> LTDC_R3                   PG10     ------> LTDC_G3
//    PJ3     ------> LTDC_R4                   PH15     ------> LTDC_G4
//    PJ4     ------> LTDC_R5                   PH4     ------> LTDC_G5
//    PJ5     ------> LTDC_R6                   PK1     ------> LTDC_G6
//    PJ6     ------> LTDC_R7                   PK2     ------> LTDC_G7
//
//    PJ12     ------> LTDC_B0
//    PJ13     ------> LTDC_B1
//    PJ14     ------> LTDC_B2
...
//	GPIO_InitStruct.Mode 	= GPIO_MODE_OUTPUT_PP;			// 推挽输出模式
//	GPIO_InitStruct.Pull 	= GPIO_NOPULL;						// 无上下拉
//	GPIO_InitStruct.Speed 	= GPIO_SPEED_FREQ_LOW;			// 速度等级高
//	HAL_GPIO_Init(LCD_Backlight_PORT, &GPIO_InitStruct);	// 初始化
//
//    LCD_Backlight_OFF;	// 先关闭背光引脚，初始化LTDC之后再开启
...
void  LCD_DisplayNumber( uint16_t x, uint16_t y, uint32_t number, uint8_t len)
{
	...
	{
		sprintf( Number_Buffer , "%.*lu",len, number );	// 将 number 转换成字符串，便于显示
	}
	else			// 多余位补空格
	{
		sprintf( Number_Buffer , "%*lu",len, number );	// 将 number 转换成字符串，便于显示
